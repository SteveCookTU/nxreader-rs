...
pub fn read_dens(client: &SysBotClient) -> Vec<Den> {
    ...
    for i in 0..DEN_COUNT {
        if i > 189 {
            dens.push(Den::new(den_data[((i + 32) * DEN_SIZE)..((i + 33) * DEN_SIZE)].to_vec()));
        } else if i > 99 {
            dens.push(Den::new(den_data[((i + 11) * DEN_SIZE)..((i + 12) * DEN_SIZE)].to_vec()));
        } else {
            dens.push(Den::new(den_data[(i * DEN_SIZE)..((i + 1) * DEN_SIZE)].to_vec()))
        }
    }
    dens
}
